...
void PanoramaStitcher::run_panorama_stitcher(Detector chosenDetector, ImagePair imagePair) {
    ...
    for (int i = 0; i < imagePaths.size()-1; i+=2)
    {
        /**
            ...
            // 4. Copy img2 region into mask2 (top-left corner = translated position)
            Rect roi_in_panorama(-bbox.x, -bbox.y, img2.cols, img2.rows);
            roi_in_panorama.x = std::max(0, roi_in_panorama.x);
            roi_in_panorama.y = std::max(0, roi_in_panorama.y);

            // Inputs: panorama (warped img1), img2 placed with translation
            // Both should already be inside the same canvas size (bbox)
            Mat blended = panorama.clone();

            // define overlap region
            Rect overlap = roi_in_panorama & Rect(0, 0, panorama.cols, panorama.rows);

            // feather blend only the overlap
            for (int y = overlap.y; y < overlap.y + overlap.height; y++) {
                for (int x = overlap.x; x < overlap.x + overlap.width; x++) {
                    Vec3b p1 = panorama.at<Vec3b>(y, x);
                    Vec3b p2 = img2.at<Vec3b>(y - roi_in_panorama.y, x - roi_in_panorama.x);

                    // weight: closer to left uses more p1, closer to right uses more p2
                    float alpha = float(x - overlap.x) / float(overlap.width);

                    Vec3b blendedPixel;
                    for (int c = 0; c < 3; c++) {
                        blendedPixel[c] = uchar((1 - alpha) * p1[c] + alpha * p2[c]);
                    }

                    blended.at<Vec3b>(y, x) = blendedPixel;
                }
            }

            // copy non-overlapping img2 parts into blended
            img2.copyTo(blended(roi_in_panorama), (panorama(roi_in_panorama) == Vec3b(0,0,0)));

            imshow("Blended panorama of " + imagePaths[i] + " & " + imagePaths[i], blended);
    }
